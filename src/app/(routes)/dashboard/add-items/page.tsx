...
const Page = () => {
  ...
  const analyzeWithAI = async (imageFile: File) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setToast({
          message:
            'AI berhasil menganalisis gambar! Klik "Terapkan Saran AI" untuk menggunakan hasil analisis.',
          type: "success",
        });
      } else {
        ...
        if (
          ...
          setToast({
            message:
              "Server AI sedang sibuk. Silakan coba lagi dalam beberapa menit atau isi form secara manual.",
            type: "error",
          });
          ...
          setToast({
            message:
              result.message ||
              "Gagal menganalisis gambar dengan AI. Silakan isi form secara manual.",
            type: "error",
          });
        }
      }
      ...
      setToast({
        message:
          "Koneksi ke server AI bermasalah. Silakan periksa koneksi internet dan coba lagi.",
        type: "error",
      });
    } finally {
  ...
  const applyAISuggestions = () => {
    ...
    const matchingCategory = kategoriList.find(
      (cat) =>
    ...
    setToast({
      message:
        "Saran AI telah diterapkan! Silakan review dan sesuaikan jika diperlukan.",
      type: "success",
    });
  };
  ...
  const handleSubmit = async () => {
    ...
    if (jenisBarang === "Sewa" && (!harga || harga <= 0))
      return setErrorMessage("Harga sewa wajib diisi dan lebih dari 0.");
    if (jenisBarang === "Thrifting" && (!harga || harga <= 0))
      return setErrorMessage("Harga thrifting wajib diisi dan lebih dari 0.");
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        setToast({
          message: result.message || "Gagal menyimpan item.",
          type: "error",
        });
      }
    } catch (err) {
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-blue-700 mt-1">
                Upload gambar pertama untuk mengaktifkan analisis AI otomatis.
                AI akan mengisi nama, kategori, deskripsi, dan detail lainnya
                secara otomatis!
              </p>
            </div>
          </div>
            ...
            <div className="flex items-start gap-3">
              <div className="flex-1">
                ...
                <div className="text-xs text-green-700 mt-2 space-y-1">
                  <p>
                    <strong>Nama:</strong> {aiSuggestions.name}
                  </p>
                  <p>
                    <strong>Kategori:</strong> {aiSuggestions.category}
                  </p>
                  <p>
                    <strong>Warna:</strong> {aiSuggestions.color}
                  </p>
                  <p>
                    <strong>Ukuran:</strong> {aiSuggestions.size}
                  </p>
                  <p>
                    <strong>Kondisi:</strong>{" "}
                    {formatAICondition(aiSuggestions.condition)}
                  </p>
                  <p>
                    <strong>Material:</strong> {aiSuggestions.material}
                  </p>
                  <p>
                    <strong>Deskripsi:</strong> {aiSuggestions.description}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-orange-700 mt-1">
                  Analisis AI gagal. Anda bisa mencoba lagi atau mengisi form
                  secara manual.
                </p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
        ...
        <div className="flex flex-col gap-y-5 mb-10">
          ...
          <CustomDropdown
            ...
            selectedValue={
              kondisiOptions.find((o) => o.value === kondisi)?.label || ""
            }
            onSelect={(label) => {
